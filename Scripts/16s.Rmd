...
sample.freq.postfilter <- read.csv("16S_seq/QIIME2_20220228/sample-frequency-detail-postfilter.csv") %>%
  rename(seq.ID = X)
...
sample.freq.postfilter %>% ggplot(aes(x=X0)) + geom_histogram(color="darkblue", fill="lightblue") + theme_bw() + xlab("read frequency per sample") +
  ylab("number of samples") + ggtitle("post-filtering")

filter(sample.freq.postfilter, X0 < 1000)
filter(sample.freq.postfilter, X0 < 1500)

sample.freq.postfilter.low <- full_join(sample.freq.postfilter, meta, by = "seq.ID") %>%
  rename(reads = X0) %>%
  mutate(read.status = case_when(reads <= 1500 ~ "low", 
                           reads > 1500 ~ "over")) %>% subset(read.status == "low") 
sample.freq.postfilter.low$Plug_ID <- as.factor(sample.freq.postfilter.low$Plug_ID)

sample.freq.postfilter.low %>% subset(Species == "Pacuta") %>%
  ggplot(aes(x=Plug_ID, y=reads), color=Treatment, group = Treatment) + geom_point() +
