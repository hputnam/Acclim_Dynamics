...
df.chl <- df.chl %>%
  mutate(chla.ug.cm2 = (chla.ug.ml * vol_mL) / surface.area.cm2,
         ...
         chla.ug.cells.cm2 = (chla.ug.ml * vol_mL) / haemo.cells.cm2,
         chlc2.ug.cells.cm2 = (chlc2.ug.ml * vol_mL) / haemo.cells.cm2)
...
Chl_final %>%
  select(Plug_ID, Species, Timepoint, Temperature, Treatment, CO2, chla.ug.cm2, chlc2.ug.cm2, chla.ug.cells.cm2, chlc2.ug.cells.cm2) %>%
...
## use the following 
Chl_long <- Chl_final %>% select(Plug_ID, surface.area.cm2, vol_mL, chla.ug.cm2, chlc2.ug.cm2, chla.ug.cells.cm2, chlc2.ug.cells.cm2, Species, Treatment, Temperature, CO2, Timepoint) %>% gather("Measurement", "Value", 4:7)
...
Chl_means <- Chl_means %>% subset(!Timepoint == "Day 1" & !Timepoint == "Day 2") %>% 
  subset(!Timepoint == "12 week" | !Treatment == "HTAC" | !Species == "Pocillopora acuta")


Chl_means_cm2 <- Chl_means %>% subset(Measurement == "chla.ug.cm2" | Measurement == "chlc2.ug.cm2")
Chl_means_cells_cm2 <- Chl_means %>% subset(Measurement == "chla.ug.cells.cm2" | Measurement == "chlc2.ug.cells.cm2")
...
CHL.cells.cm2 <- ggplot(Chl_means_cells_cm2, aes(x=Timepoint, y = Value, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  facet_grid(Measurement ~ Species, scales = "free") +
  geom_errorbar(aes(ymin=Value-se, ymax=Value+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
  theme_classic() + 
  ylab(expression("Concentration" ~(ug ~cells.cm^-2))) +
  #ggtitle("Chlorophyll a and c2 concentration") +
  theme(legend.position = "left") +
  geom_vline(xintercept = c(5.2), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  ...
  strip.text.x = element_text(size = 12, color = "black", face = "bold.italic"),
  # Change axis line
  axis.line = element_line(colour = "black")); CHL.cells.cm2


CHL.site <- ggplot(Chl_final, aes(x=Timepoint, y = chla.ug.cm2, group = Site.Name, color = Site.Name)) +
  geom_line() + geom_point() +
  facet_grid(Treatment ~ Species, scales = "free") +
  #geom_errorbar(aes(ymin=Value-se, ymax=Value+se), width=.2) +
  #scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
  theme_classic() + 
  ...
  strip.text.x = element_text(size = 12, color = "black", face = "bold.italic"),
  # Change axis line
  axis.line = element_line(colour = "black")); CHL.site
...
ggsave(file="Output/Final_Figures/Chlorophyll_cells.pdf", CHL.cells.cm2, width = 11, height = 6, units = c("in"))
ggsave(file="Output/Final_Figures/Chlorophyll_cells.png", CHL.cells.cm2, width = 11, height = 6, units = c("in"))
```
...
```{r}
Mcap_chl <- Chl_final %>% subset(Species == "Pacuta")
Pacuta_chl <- Chl_final %>% subset(Species == "Mcapitata")

mcap.chl.stress <- Mcap_chl %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week")
pacuta.chl.stress <- Pacuta_chl %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week")

mcap.chl.recovery <- Mcap_chl %>% subset(Timepoint == "12 week" | Timepoint == "16 week")
pacuta.chl.recovery <- Pacuta_chl %>% subset(Timepoint == "12 week" | Timepoint == "16 week") %>% subset(Treatment == "ATAC" | Treatment == "ATHC")

hist(mcap.chl.recovery$chla.ug.cm2) 
hist(mcap.chl.stress$chla.ug.cm2) 

hist(pacuta.chl.recovery$chla.ug.cm2) 
hist(pacuta.chl.stress$chla.ug.cm2) 

hist(mcap.chl.recovery$chlc2.ug.cm2) # transform this one 
hist(mcap.chl.stress$chlc2.ug.cm2) 

hist(pacuta.chl.recovery$chlc2.ug.cm2) # transform this one 
hist(pacuta.chl.stress$chlc2.ug.cm2) 
```
...
```{r}
## Chlorophyll a
Pacuta_Chl_LMER_stress <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta.chl.stress)
Pacuta_Chl_LMER_recovery <- lmer(chla.ug.cm2 ~ Timepoint*CO2 + (1|Tank), na.action=na.omit, data=pacuta.chl.recovery)

qqPlot(residuals(Pacuta_Chl_LMER_stress)) 
qqPlot(residuals(Pacuta_Chl_LMER_recovery)) 

hist(residuals(Pacuta_Chl_LMER_stress)) 
hist(residuals(Pacuta_Chl_LMER_recovery)) 

## Type III Wald chisquare tests
pacuta.chl.stress.anova <- Anova(Pacuta_Chl_LMER_stress, ddf="lme4", type='III')
pacuta.chl.recovery.anova <- Anova(Pacuta_Chl_LMER_recovery, ddf="lme4", type='III')

capture.output(pacuta.chl.stress.anova, file = "Output/Statistics/chl_stats_pacuta_stress.txt")
capture.output(pacuta.chl.recovery.anova, file = "Output/Statistics/chl_stats_pacuta_recovery.txt")

## Chlorophyll c2
Pacuta_Chl_LMER_stress_c2 <- lmer(chlc2.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta.chl.stress)
Pacuta_Chl_LMER_recovery_c2 <- lmer(log(chlc2.ug.cm2) ~ Timepoint*CO2 + (1|Tank), na.action=na.omit, data=pacuta.chl.recovery)

qqPlot(residuals(Pacuta_Chl_LMER_stress_c2)) 
qqPlot(residuals(Pacuta_Chl_LMER_recovery_c2)) 

hist(residuals(Pacuta_Chl_LMER_stress_c2)) 
hist(residuals(Pacuta_Chl_LMER_recovery_c2)) 

## Type III Wald chisquare tests
pacuta.chl.stress.c2.anova <- Anova(Pacuta_Chl_LMER_stress_c2, ddf="lme4", type='III')
pacuta.chl.recovery.c2.anova <- Anova(Pacuta_Chl_LMER_recovery_c2, ddf="lme4", type='III')

capture.output(pacuta.chl.stress.c2.anova, file = "Output/Statistics/chl_c2_stats_pacuta_stress.txt")
capture.output(pacuta.chl.recovery.c2.anova, file = "Output/Statistics/chl_c2_stats_pacuta_recovery.txt")
```
...
```{r}
## Chlorophyll a
Mcap_Chl_LMER_stress <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap.chl.stress)
Mcap_Chl_LMER_recovery <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap.chl.recovery)

qqPlot(residuals(Mcap_Chl_LMER_stress)) 
qqPlot(residuals(Mcap_Chl_LMER_recovery)) 

hist(residuals(Mcap_Chl_LMER_stress)) 
hist(residuals(Mcap_Chl_LMER_recovery)) 

## Type III Wald chisquare tests
mcap.chl.stress.anova <- Anova(Mcap_Chl_LMER_stress, ddf="lme4", type='III')
mcap.chl.recovery.anova <- Anova(Mcap_Chl_LMER_recovery, ddf="lme4", type='III')
