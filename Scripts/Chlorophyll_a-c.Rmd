...
# Corals we won't have Chl_IDs for: Zero 
# output excluding those is 2156
# need to re-do 2156
```
...
Chl_means <- summarySE(Chl_long, measurevar="Value", groupvars=c("Timepoint", "Treatment", "Temperature", "CO2", "Species", "Measurement"))
...
Plotting timeseries
## BELOW IS USING THE INCORRECT MEANS TABLE. RE-VISIT TO TAKE THIS OUT 
```{r}
...
species.names <- list(
  ...
  'chla.ug.cm2'="Chl-a (ug/cm^-2)",
  'chlc2.ug.cm2'="Chl-c2 (ug/cm^-2)")
...
CHL <- ggplot(Chl_means, aes(x=Timepoint, y = Value, group = Treatment, color = Treatment)) +
  ...
  facet_grid(Measurement ~ Species, scales = "free", labeller = species_labeller) +
...
```{r}
## Chlc2 
Mcap.chlc <- Chlc2_means %>% subset(Species=="Mcapitata")
Pacuta.chlc <- Chlc2_means %>% subset(Species=="Pacuta")
  
Mcap.Chlc2.fig <- ggplot(Mcap.chlc, aes(x=Timepoint, y = chlc2.ug.cm2, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=chlc2.ug.cm2-se, ymax=chlc2.ug.cm2+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
  ylab(expression(Chlc2 ~(ug ~cm^2))) +
  theme_bw() + 
  ylim(0,1.25) +
  ggtitle("Montipora capitata") +
  theme(legend.position = c(), legend.title = element_text(size=8), legend.text = element_text(size=7), legend.background = element_blank(), 
       legend.key = element_blank()) +
  geom_vline(xintercept = c(7.35), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1.2, hjust = 1.3)) #Set the text angle 
Mcap.Chlc2.fig

Pacuta.Chlc2.fig <- ggplot(Pacuta.chlc, aes(x=Timepoint, y = chlc2.ug.cm2, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=chlc2.ug.cm2-se, ymax=chlc2.ug.cm2+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
  ...
  theme(legend.position = c(0.9,0.8), legend.title = element_text(size=8), legend.text = element_text(size=7), legend.background = element_blank(), 
        legend.key = element_blank()) +
  geom_vline(xintercept = c(7.35), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1.2, hjust = 1.3)) #Set the text angle 
...
## Chla 
Mcap.chla <- Chla_means %>% subset(Species=="Mcapitata")
Pacuta.chla <- Chla_means %>% subset(Species=="Pacuta")
  
Mcap.Chla.fig <- ggplot(Mcap.chla, aes(x=Timepoint, y = chla.ug.cm2, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=chla.ug.cm2-se, ymax=chla.ug.cm2+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
...
Pacuta.Chla.fig <- ggplot(Pacuta.chla, aes(x=Timepoint, y = chla.ug.cm2, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=chla.ug.cm2-se, ymax=chla.ug.cm2+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
...
Chl.Figs2 <- arrangeGrob(Mcap.Chlc2.fig, Pacuta.Chlc2.fig, Mcap.Chla.fig, Pacuta.Chla.fig, ncol=2)
ggsave(file="Output/Final_Figures/Chlorophyll-final2.pdf", Chl.Figs2, width = 11, height = 6, units = c("in"))
ggsave(file="Output/Final_Figures/Chlorophyll-final2.png", Chl.Figs2, width = 11, height = 6, units = c("in"))
```
