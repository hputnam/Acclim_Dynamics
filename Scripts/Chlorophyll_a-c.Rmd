...
Master_fragment <- read.csv("Environmental_data/Master_Fragment.csv", header=T, sep=",", na.string="NA") %>% 
  filter(!ANALYSIS == "Molecular") %>% filter(!ANALYSIS == "Bleaching") %>% filter(!ANALYSIS == "NA") %>% filter(!ANALYSIS == "Molecular Melted") %>% 
  filter(!ANALYSIS == "Molecular BB") %>% filter(!ANALYSIS == "Physiology BB") %>% # need to keep "Physiology" and "Physiology/Molecular"
...
CHL <- ggplot(Chl_means, aes(x=Timepoint, y = Value, group = Treatment, color = Treatment)) +
  ...
  ylab("") +
  theme_bw() + 
  ggtitle("Chlorophyll a/c concentration (ug/cm^-2)") +
  theme(legend.position = "left") +
  geom_vline(xintercept = c(7.2), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5, hjust = 0.45)) #Set the text angle
CHL
