...
Pacuta_chl <- Chl_final %>% subset(Species == "Pacuta")

hist(Mcap_chl$chla.ug.cm2) # right skewed
hist(Mcap_chl$chlc2.ug.cm2) # right skewed 
...
Pacuta_chl <- Pacuta_chl %>% filter(Timepoint !="12 week" & Timepoint !="16 week")

Pacuta_Chl_LMER <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=Pacuta_chl) #GLMM
Pacuta_Chl_LMER.reef <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank) + (1|Site.Name), na.action=na.omit, data=Pacuta_chl)
model.sel(Pacuta_Chl_LMER, Pacuta_Chl_LMER.reef) # model selection function 
## With reef AIC = 376.9; without reef AIC = 384.8. 
...
```{r}
Mcap_Chl_LMER <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=Mcap_chl) 
Mcap_Chl_LMER.reef <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank) + (1|Site.Name), na.action=na.omit, data=Mcap_chl)
