---
title: "photosynthetic_irradiance_curves.Rmd"
author: "Emma Strand"
date: "2023-07-20"
...
library('plyr')
library('dplyr')
# not available on cran anymore: library('phytotools')
library("broom")
library(Hmisc)
library(Rmisc)
...
metadata <- metadata %>%
  select(colony_id, Species, Run, Chamber.Vol.L, Date, Start.time, Stop.time, Light_Value)
...
```{r}
# Define data  
Data <- pr.out #read.csv("output/PI_curves/pi_curve_rates.csv", sep = ",")
...
### left off here 
  qplot(PAR, Pc, data = ., geom = 'point', group = colony_id) +
  geom_line(aes(y=.fitted)) + facet_grid(Date~Species) + theme_bw() +
  scale_shape_manual(21) + 
  theme(legend.position="right")  

ggsave("output/PI_Curves/curves_aggregated.pdf", plot = last_plot(), width = 12, height = 8, units ="in")
...
```{r}
pars <- nls_data %>% #read_csv("output/2_pi_curve_pars_nls.csv") 
...
ggsave("output/PI_Curves/Metrics.png", plot = last_plot(), width = 12, height = 5, units ="in")
```

## 15. Plot PI curves together 
...
Data <- Data %>%
  ...
  mutate(Species = case_when(
    Species == "MC" ~ "Mcapitata",
    Species == "PA" ~ "Pacuta"
  )) %>% left_join(., md, by = c("Plug_ID", "Species"))

Data$PAR <- as.numeric(Data$Light_Value)

PA <- Data %>% subset(Species == "Pacuta")
MC <- Data %>% subset(Species == "Mcapitata")
  
plot(PA$PAR, PA$micromol.cm2.h,
     xlab="", ylab="", 
     xlim=c(0,max(PA$PAR)), cex.lab=0.8,cex.axis=0.8,cex=1, main="A) P. acuta", adj = 0.05) #set plot info
...
PA.mod <- fitJP(PA$PAR, PA$micromol.cm2.h, 
                normalize = FALSE, lowerlim = c(0, 1), upperlim = c(100, 1000), fitmethod=c("Nelder-Mead"))
A.PA <- PA.mod$alpha[1]
A.PA
Ik.PA <- PA.mod$ek[1]
Ik.PA
