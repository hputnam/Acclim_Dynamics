...
### Subsetting data 
```{r}
## Stress and recovery period (no resp)
full.data <- data %>% select(-Pnet_umol.cm2.hr, -Rdark_umol.cm2.hr, -Pgross_umol.cm2.hr)
full.data.pca <- full.data[complete.cases(full.data), ] 

# The above dataframes split Species
Mcap.full.pca <- full.data.pca %>% subset(Species == "Mcapitata")
Pacuta.full.pca <- full.data.pca %>% subset(Species == "Pacuta")

## Full data stress period (resp and photo)
stress.data <- data %>% subset(!Timepoint == "12 week" & !Timepoint == "16 week")
stress.data <- stress.data[complete.cases(stress.data), ]  

# Stress time period split by species 
Mcap.stress.pca <- stress.data %>% subset(Species == "Mcapitata")
Pacuta.stress.pca <- stress.data %>% subset(Species == "Pacuta")
```
...
Mcap_group <- autoplot(Mcap_scaled_stress_data, data = Mcap.stress.pca, colour = 'Treatment', shape = "Timepoint",
         loadings = TRUE, loadings.colour = 'black', size=1, loadings.label = TRUE, 
         loadings.label.size = 3, loadings.label.colour = 'black', loadings.label.vjust=-1,
         frame=TRUE, frame.type = 'norm') + theme_bw() +
  scale_color_manual(values = line.cols) + scale_fill_manual(values = fill.cols) + ggtitle("M. capitata"); Mcap_group

Pacuta_group <- autoplot(Pacuta_scaled_stress_data, data = Pacuta.stress.pca, colour = 'Treatment', shape = "Timepoint", frame=TRUE, frame.type = 'norm',
         loadings = TRUE, loadings.colour = 'black', size=1, loadings.label = TRUE, 
         loadings.label.size = 3, loadings.label.colour = 'black', loadings.label.vjust=-1) + 
