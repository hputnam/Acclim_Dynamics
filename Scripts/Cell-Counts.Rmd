...
cellometer_sym_counts <- cellometer %>%
  select(Date, Plug_ID, matches("Cell_mL[0-3]")) %>%
  gather("rep", "count", -Plug_ID, -Date) %>%
...
# Plot data
```{r}
counts <- full_join(cellometer_sym_counts, haemo_sym_counts, join_by = "Plug_ID") %>% filter(!is.na(count.mean)) %>% 
...
write_csv(path = "Physiology_variables/cell-counts.csv", counts)

cellometer.testing <- counts %>% subset(!Date == "Spring-2021")
  
counts_lm <- cellometer.testing %>%
  group_by(Species) %>%
  nest() %>% # nest to do this for eachgroup (Species) defined above 
...
testing <- cellometer.testing %>%
  ggplot(aes(x=cellometer.cells.cm2, y=haemo.cells.cm2, group = Date, color = Date)) + 
  geom_jitter(width = 0.1) + 
  facet_grid(~Species) +
  labs(color='Cellometer Date') +
  theme_classic() + 
  ylim(0, 3000000) + 
  xlim(0, 3000000) +
  geom_abline(slope=1) +
