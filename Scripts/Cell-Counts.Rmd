...
count.means <- count.means %>% subset(!Timepoint == "Day 1" & !Timepoint == "Day 2") %>% filter(N > 12) # N is higher because grouping number of counts in previous dataframe. mean here is still mean of the pop where each individual has an average count of six counts
```
...
ggsave(file="Output/Final_figures/Cell_counts.png", cellcounts_plot, width = 11, height = 8, units = c("in"))
```
...
mcap.counts.stress <- mcap.counts %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week")
mcap.counts.recovery <- mcap.counts %>% subset(Timepoint == "10 week" | Timepoint == "12 week" | Timepoint == "14 week" | Timepoint == "16 week")

pacuta.counts.stress <- pacuta.counts %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week")
pacuta.counts.recovery <- pacuta.counts %>% subset(Timepoint == "10 week" | Timepoint == "12 week" | Timepoint == "14 week" | Timepoint == "16 week") %>% subset(Treatment == "ATAC" | Treatment == "ATHC")

hist(mcap.counts.stress$haemo.cells.cm2) 
hist(mcap.counts.recovery$haemo.cells.cm2) 
hist(pacuta.counts.stress$haemo.cells.cm2) 
hist(pacuta.counts.recovery$haemo.cells.cm2) 
```

Model selection 

```{r}
Pacuta_counts_LMER_stress <- lmer(log(haemo.cells.cm2) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta.counts.stress) 
Pacuta_counts_LMER_recovery <- lmer(haemo.cells.cm2 ~ Timepoint*CO2 + (1|Tank), na.action=na.omit, data=pacuta.counts.recovery) 

Mcap_counts_LMER_stress <- lmer(haemo.cells.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap.counts.stress) 
Mcap_counts_LMER_recovery <- lmer(haemo.cells.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap.counts.recovery) 

qqPlot(residuals(Pacuta_counts_LMER_stress)) 
qqPlot(residuals(Pacuta_counts_LMER_recovery)) 
qqPlot(residuals(Mcap_counts_LMER_stress)) 
qqPlot(residuals(Mcap_counts_LMER_recovery)) 

hist(residuals(Pacuta_counts_LMER_stress)) 
hist(residuals(Pacuta_counts_LMER_recovery)) 
hist(residuals(Mcap_counts_LMER_stress)) 
hist(residuals(Mcap_counts_LMER_recovery)) 

## Type III Wald chisquare tests
pacuta.counts.stress.anova <- Anova(Pacuta_counts_LMER_stress, ddf="lme4", type='III') 
pacuta.counts.recovery.anova <- Anova(Pacuta_counts_LMER_recovery, ddf="lme4", type='III') 

mcap.counts.stress.anova <- Anova(Mcap_counts_LMER_stress, ddf="lme4", type='III') 
mcap.counts.recovery.anova <- Anova(Mcap_counts_LMER_recovery, ddf="lme4", type='III') 

capture.output(pacuta.counts.stress.anova, file = "Output/Statistics/Counts_stress_pacuta.txt")
capture.output(pacuta.counts.recovery.anova, file = "Output/Statistics/Counts_recovery_pacuta.txt")
capture.output(mcap.counts.stress.anova, file = "Output/Statistics/Counts_stress_mcap.txt")
capture.output(mcap.counts.recovery.anova, file = "Output/Statistics/Counts_recovery_mcap.txt")
```
