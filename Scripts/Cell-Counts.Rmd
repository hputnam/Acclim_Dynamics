...
```{r}
# Surface area data
sa <- read.csv("Physiology_variables/Respirometry/coral_surface_area.csv", sep = ",") %>% 
...
homog_vols <- read.csv("Physiology_variables/homogenate_vol.csv", sep = ",") %>% select(Species, Plug_ID, vol_mL)
homog_vols$Plug_ID <- as.character(homog_vols$Plug_ID)

# Coral sample metadata
metadata <- read.csv("Environmental_data/Master_Fragment.csv", sep = ",") %>% select(Species, Plug_ID, Tank, Treatment, Temperature, CO2, Timepoint, ANALYSIS, Sample.Date) %>%
...
```{r}
## Haemocytometer data 
haemo.raw <- read.csv("Physiology_variables/cell-counts/cell_counts.csv")
...
```{r}
cellcounts_plot <- ggplot(count.means.test.stress, aes(x=Timepoint, y = haemo.cells.cm2, group = Treatment, color = Treatment)) +
  ...
  theme(axis.text.x = element_text(size = 10, angle = 60, vjust = 1.2, hjust = 1.3)) + #Set the text angle
...
ggsave(file="Output/Manuscript_figures/Counts_stress.png", cellcounts_plot, width = 11, height = 6, units = c("in"))
...
cellcounts_plot_recovery <- ggplot(count.means.test.recovery, aes(x=Timepoint, y = haemo.cells.cm2, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  facet_grid(~Species) +
  geom_errorbar(aes(ymin=haemo.cells.cm2-se, ymax=haemo.cells.cm2+se), width=.2) +
  scale_color_manual(values = cols) + 
...
ggsave(file="Output/Manuscript_figures/cellcounts_recovery.png", cellcounts_plot_recovery, width = 5, height = 4, units = c("in"))
```
