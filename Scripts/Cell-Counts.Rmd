...
haemo <- haemo.raw %>%
  select(Haemocytometer_Date, Plug_ID, Squares_Counted, matches("Count[0-9]"), Tech) %>%
  gather("rep", "count", -Haemocytometer_Date, -Plug_ID, -Squares_Counted, -Tech) %>%
...
Count1 vs Count2
```{r}
cellometer.count.comparison <- cellometer %>% select(Plug_ID, Species, rep, count, Date, Period) %>% 
  spread(rep, count)
  
c1c2.lm <- cellometer.count.comparison %>% subset(Period == "Pre-homogenizing" | Period == "Post-homogenizing") %>%
...
c1c2 <- cellometer.count.comparison %>% subset(Period == "Pre-homogenizing" | Period == "Post-homogenizing") %>%
...
# combining rows that have the same data (plug ids with cellometer and haemcy but originally on two different rows)

write_csv(path = "Physiology_variables/cell-counts/calculated-cell-counts.csv", counts)
```

Creating model for regression between cellometer and haemocytometer.
```{r}
