...
BELOW IS TEMPORARY WHILE MISSING CORALS ARE TRACKED DOWN
```{r}
...
# Changing the date column to be a factor not integer. 
Mcap$Date <- as.factor(Mcap$Date)
Pact$Date <- as.factor(Pact$Date)
```
...
## Montipora capitata
Mcap.Pg <- ggplot(Mcap, aes(x=Date, y = Pgross.mean, group = Treatment, color = Treatment)) +
...
Mcap.Pn <- ggplot(Mcap, aes(x=Date, y = Pnet.mean, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  ...
  ylim(c(0, 3)) + # y-axis label and y axis range
  theme_classic()+
  theme(legend.position = "none",axis.title.x = element_blank(), axis.text.x=element_blank(), plot.title = element_text(face="bold"))
Mcap.Pn

Mcap.Rd <- ggplot(Mcap, aes(x=Date, y = Rdark.mean, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() + 
...
## Pocillopora acuta
Pact.Pg <- ggplot(Pact, aes(x=Date, y = Pgross.mean, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=Pgross.mean-Pgross.se, ymax=Pgross.mean+Pgross.se), width=.2) +
  scale_color_manual(values = colors) + 
  xlab("Date") + 
  ...
  labs(title = "P. acuta") +
  theme_classic() + 
  theme(legend.position = c(0.8, 0.8), axis.title.x = element_blank(), axis.text.x=element_blank(),
        plot.title = element_text(face="bold.italic", hjust=0.5, size=20)) 
Pact.Pg

Pact.Pn <- ggplot(Pact, aes(x=Date, y = Pnet.mean, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=Pnet.mean-Pnet.se, ymax=Pnet.mean+Pnet.se), width=.2) +
  scale_color_manual(values = colors) + 
  xlab("Date") + theme(axis.text.x=element_text(angle=60, hjust=1)) + # x-axis label and rotating text on x axis
...
Pact.Rd <- ggplot(Pact, aes(x=Date, y = Rdark.mean, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin=Rdark.mean-Rdark.se, ymax=Rdark.mean+Rdark.se), width=.2) +
  scale_color_manual(values = colors) + 
  xlab("Date") + 
...
ggsave(file="Output/Respirometry.pdf", Resp.Figs, width = 11, height = 10, units = c("in"))
ggsave(file="Output/Respirometry.jpg", Resp.Figs, width = 11, height = 10, units = c("in"))
...
Pact.Pnet.aov <- aov(Pnet_umol.cm2.hr ~ Treatment*Date.x, data = Pact.All)
...
Pact.Pgross.aov <- aov(Pgross_umol.cm2.hr ~ Treatment*Date.x, data = Pact.All)
...
Pact.Rdark.aov <- aov(Rdark_umol.cm2.hr ~ Treatment*Date.x, data = Pact.All)
...
## M. capitata
# Net Photosynthetic Rates
Mcap.Pnet.aov <- aov(log10(Pnet_umol.cm2.hr) ~ Treatment*Date.x, data = Mcap.All)
...
# Gross Photosynthetic Rates
Mcap.Pgross.aov <- aov(log10(Pgross_umol.cm2.hr) ~ Treatment*Date.x, data = Mcap.All)
...
# Dark Respiration Rates
Mcap.Rdark.aov <- aov(Rdark_umol.cm2.hr ~ Treatment*Date.x, data = Mcap.All)
