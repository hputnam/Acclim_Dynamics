...
Means <- summarySE(Missing, measurevar="Value", groupvars=c("Species.x", "Date.x", "Treatment", "Measurement")) %>% 
  filter(N>2) %>% na.omit() %>% dplyr::rename(Date = Date.x) %>% dplyr::rename(Species = Species.x)
