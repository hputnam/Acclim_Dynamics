...
ggsave(file="Output/Final_Figures/Respirometry_Pnet_photo_cells.pdf", Photo.cells.plot, width = 8, height = 3, units = c("in"))
ggsave(file="Output/Final_Figures/Respirometry_Pnet_photo_cells.png", Photo.cells.plot, width = 8, height = 5, units = c("in"))
```

### Statistics on umol.cells.hr

```{r}
Photo.cells.stats <- Photo.cells %>% spread(Measurement, value)
  
Pnet_cells_lmer <- lmer(log(umol.cells.hr) ~ Timepoint*Temperature*CO2*Species + (1|Tank), na.action=na.omit, data=Photo.cells.stats) 
qqPlot(residuals(Pnet_cells_lmer))
hist(residuals(Pnet_cells_lmer)) 

pnet_cells_anova <- Anova(Pnet_cells_lmer, ddf="lme4", type='III') # Type III Wald chisquare tests
capture.output(pnet_cells_anova, file = "Output/Statistics/Pnet_cells.txt")
```
...
```{r}
Pacuta_Pnet_LMER <- lmer(sqrt(Pnet_umol.cm2.hr) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta_photoresp) 
Mcap_Pnet_LMER <- lmer(sqrt(Pnet_umol.cm2.hr) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap_photoresp) 
...
tab_model(Pacuta_Pnet_LMER, Mcap_Pnet_LMER,
          dv.labels = c("P.acuta Pnet", "M. cap Pnet"), string.ci = "Conf. Int (95%)",
          string.p = "P-Value", file = "Output/Statistics/Pnet.doc")

capture.output(pacuta_pnet_anova, file = "Output/Statistics/Pnet_Pacuta.txt")
capture.output(mcap_pnet_anova, file = "Output/Statistics/Pnet_Mcap.txt")
```

Gross photosynthetic rates. 
```{r}
Pacuta_Pgross_LMER <- lmer((Pgross_umol.cm2.hr) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta_photoresp) 
Mcap_Pgross_LMER <- lmer(log(Pgross_umol.cm2.hr) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap_photoresp) 
...
tab_model(Pacuta_Pgross_LMER, Mcap_Pgross_LMER,
          dv.labels = c("P.acuta Pgross", "M. cap Pgross"), string.ci = "Conf. Int (95%)",
          string.p = "P-Value", file = "Output/Statistics/Pgross.doc")

capture.output(pacuta_pgross_anova, file = "Output/Statistics/Pgross_Pacuta.txt")
capture.output(mcap_pgross_anova, file = "Output/Statistics/Pgross_Mcap.txt")
```

Respiration rates. 

```{r}
Pacuta_Resp_LMER <- lmer(Rdark_umol.cm2.hr ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=pacuta_photoresp) 
Mcap_Resp_LMER <- lmer((Rdark_umol.cm2.hr) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=mcap_photoresp) 
...
capture.output(pacuta_resp_anova, file = "Output/Statistics/Resp_Pacuta.txt")
capture.output(mcap_resp_anova, file = "Output/Statistics/Resp_Mcap.txt")
```
