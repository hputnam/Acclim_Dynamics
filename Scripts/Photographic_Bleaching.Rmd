...
```{r}
Data <- read.csv("Physiology_variables/Bleaching_Intensity/Bleaching_ImageJ_QC.csv", header=T, sep=",", na.string="NA") #read in data file
Sample.Info <- read.csv("Environmental_data/Master_Fragment_20200616_QC.csv", header=T, sep=",", na.string="NA") #read in data file 
...
```{r}
write.table(Blch, file = "Physiology_variables/Bleaching_Intensity/Bleaching_Score.csv", append = FALSE, quote = TRUE, sep = ",",
...
Bleaching.Score <- read.csv("Physiology_variables/Bleaching_Intensity/Bleaching_Score.csv", header=T, sep=",", na.string="NA") #read in data file

mod1 <- aov(sqrt(Bleaching.Score+200) ~ Treatment*Timepoint*Species, data=Bleaching.Score) #run an ANOVA 
hist(residuals(mod1)) #look at normality of data
boxplot(residuals(mod1)) #look at normality of data
summary(mod1)

#posthoc analyses
marginal <- emmeans(mod1, ~ Treatment*Timepoint*Species)
...
phlett <- phlett[order(phlett$Timepoint, phlett$Treatment, phlett$Species),]
phlett
```

Calculating means, sample size, and standard error for timepoint, species, and treatment groups. 
```{r}
All.Means <- ddply(Bleaching.Score, c('Timepoint','Species','Treatment'), summarize,
...
All.Means$Timepoint <- factor(All.Means$Timepoint, levels = c("Week1", "Week2", "Week3", "Week4", "Week6", "Week7", "Week8", "Week9", "Week10", "Week11", "Week12", "Week13", "Week14", "Week15", "Week16"))
```
...
pdf("Physiology_variables/Bleaching_Intensity/Bleaching_All.pdf")
print(Fig.All, newpage = FALSE)
dev.off()

png("Physiology_variables/Bleaching_Intensity/Bleaching_All.png")
print(Fig.All, newpage = FALSE)
dev.off()
```
...
Calculating M.cap means and plotting bleaching intensity
```{r}
...
Calculating P. acuta means and plotting bleaching intensity
```{r}
...
```{r}
Mcap <- subset(Blch, Species=="Montipora")
Pacuta <- subset(Blch, Species=="Pocillopora")
# Identifying outliers
Mcap %>% group_by(Treatment, Timepoint) %>% identify_outliers(Bleaching.Score)
Pacuta %>% group_by(Treatment, Timepoint) %>% identify_outliers(Bleaching.Score)
# Normality Assumption 
Mcap %>% group_by(Treatment, Timepoint) %>% shapiro_test(Bleaching.Score)
Pacuta %>% group_by(Treatment, Timepoint) %>% shapiro_test(Bleaching.Score)
# QQ plot 
ggqqplot(Mcap, "Bleaching.Score", ggtheme = theme_bw()) + facet_grid(Timepoint ~ Treatment, labeller = "label_both")
```

Two-way repeated measures ANOVA. 
```{r}
Mcap.aov <- anova_test(data = Mcap, dv = Bleaching.Score, wid = PLUG.ID, within = c(Treatment, Timepoint))
get_anova_table(Mcap.aov)
## checking residuals for normality and outlier assumptions
