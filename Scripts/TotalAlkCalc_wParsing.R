...
path<-"Data/TA/20181125" #the location of all your titration files
massfile<-"20181125mass_Run2.csv" # name of your file with masses
titrationfile<-'20181125_Run2.csv'# name of the last titration file run
...
#  Data<-read.csv(file.names[i], header=F, sep=",", na.string="NA",as.is=T, skip=10)[ ,1:5] 
 # colnames(Data) <-  c("Volume","Time",	"mV",	"Temperature",	"dV/dt")
  ...
  d<-(-0.00000370*mean(Data$Temperature[mV], na.rm=T)^2-0.0001233*mean(Data$Temperature[mV], na.rm=T)+1.02900) #bottle changed 20181117
  #20180529 batch A3
  #20181117 and on, use batch A14 equations
  
  #concentration of your titrant: CHANGE EVERYTIME ACID IS CHANGED 
  # c<-0.099793 #20180529 batch A3
  c<-0.100183 #20181117 batch A14
