...
Sample.Info <- read.csv(file="data/Physiology_variables/PI_Curves/PI_Curve_Sample_Info_metadata.csv", 
                        header=T) #read sample.info data
...
for(i in 1:length(file.names.full)) { # for every file in list start at the first and run this following function
  ...
  Photo.Data1 <-read.table(file.path(path.p, "BK-1_20181017.csv"), #file.names.full[i] "BK-1_20181017.csv"
                           header=T, sep=",", skip=1, na.string="NA", 
                           fill = TRUE, as.is=TRUE, fileEncoding="latin1") 
  
  #subset columns of interest
  Photo.Data1 <- Photo.Data1 %>% dplyr::select(Time, Value, Temp) %>%
         # Photo.Data1  <- as.data.frame(cbind(Photo.Data1$Time, Photo.Data1$Value, Photo.Data1$Temp)) 
         # colnames(Photo.Data1) <- c("Time", "Value", "Temp")
    #convert time from character to time
    mutate(Time = as_hms(as.POSIXct(Time, format="%H:%M:%S")))
          # Photo.Data1$Time <- as.POSIXct(Photo.Data1$Time, format="%H:%M") 
    
  brk <- unique(as_hms(as.POSIXct(Sample.Info$Start.time, format="%H:%M")))
  ...
  for(j in 1:length(lt.levs)){    
    ...
    #start at data point ~2 minute in to avoid excess noise from start of run and remove last 3 lines containing text
    ...
    rename <- file.names[i]  #sub("_.*", "", file.names[i])
    pdf(paste0("output/PI_Curves/Output/Data_thinning/",rename,"_",j,"thinning.pdf"))
    ...
    pdf(paste0("output/PI_Curves/Output/Regressions/",rename,"_",j,"regression.pdf"))
    plot(Regs)
    dev.off()
