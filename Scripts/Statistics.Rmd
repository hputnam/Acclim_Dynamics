...
Pacuta_Chl_LMER <- lmer(chla.ug.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=Pacuta_stress_data) #GLMM
# GAM 
# anova(GLMM, GAM) # report AIC values 
...
## checking homogeneity of variance using levene test (can also use Bartlett's test function)
leveneTest(Pacuta_stress_data$chla.ug.cm2 ~ Pacuta_stress_data$Timepoint*Pacuta_stress_data$Temperature*Pacuta_stress_data$CO2)
...
## Host 
Pacuta_Host_LMER <- lmer(Host_AFDW.mg.cm2 ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=Pacuta_stress_data) #GLMM
# GAM 
# anova(GLMM, GAM) # report AIC values
...
## subset model by phase; if each individual model looks better, reporting this may be better or incorporate phase as random effect 
...
qqPlot(residuals(Mcap_Color_LMER_tank_plug))
...
### revisit row 200 and 199 and see if those are outliers or to take out; remove rows as pipe function in model
