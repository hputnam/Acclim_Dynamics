...
Select only end of acclimation period to determine the percent alive in each species at the end of that period. 

```{r}
acclimation <- Data %>% select(Species, PLUG.ID, Acclimation.End, Tank) 
acclimation <- full_join(acclimation, Tank.Info, by = "Tank") 

mont.acc <- acclimation %>% subset(Species == "Mcapitata")
poc.acc <- acclimation %>% subset(Species == "Pacuta")

mont.acc <- mont.acc %>% mutate(alive.count = nrow(subset(mont.acc,Acclimation.End == "alive")),
                                dead.count = nrow(subset(mont.acc,Acclimation.End == "dead")),
                                total.count = nrow(mont.acc),
                                percentage.alive = alive.count/total.count)

poc.acc <- poc.acc %>% mutate(alive.count = nrow(subset(poc.acc,Acclimation.End == "alive")),
                                dead.count = nrow(subset(poc.acc,Acclimation.End == "dead")),
                                total.count = nrow(poc.acc),
                                percentage.alive = alive.count/total.count)
```
...
mc <- coxme(Surv(lifespan, status) ~ Temperature*CO2 + (1|Tank/PLUG.ID), data = Mc.Data)
...
## export statistics 
capture.output(mc, file = "Output/Statistics/survivorship_coxme_mcap.txt")
capture.output(mc.anova, file = "Output/Statistics/survivorship_anova_mcap.txt")
...
pa <- coxme(Surv(lifespan, status) ~ Temperature*CO2 + (1|Tank/PLUG.ID), data = Pa.Data)
pa.anova <- Anova(pa, type = "III") 

capture.output(pa, file = "Output/Statistics/survivorship_coxme_pacuta.txt")
capture.output(pa.anova, file = "Output/Statistics/survivorship_anova_pacuta.txt")
```
...
splots[[1]] <- ggsurvplot(MC.sur, data=Mc.Data, size = 1,  # change line size
...
splots[[1]]$plot <- splots[[1]]$plot +
  #scale_x_continuous(breaks = sort(c( 8,15,29,43,57,85,112)), labels = c("1 week", "2 week", "4 week", "6 week", "8 week", "12 week", "16 week")) + 
  geom_vline(xintercept = 61, linetype = "dotted", size=2) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1.2, hjust = 1.3, size=12)) +
...
ggsave("Output/Final_Figures/All_survivorship.png", surv, width = 12, height = 6)
