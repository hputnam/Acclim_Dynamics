...
TAC <- left_join(TAC, metadata) %>%
  mutate(CRE.trial = CRE.umol.L/prot_ug.mL) %>%
  mutate(CRE.uM.mgprot = (((CRE.umol.L*(1/prot_ug.mL))/1000)*1000*1000)) 

## CRE output (mmol/L) divided by protein value (ug/mL)
## Divided by 1000 to cancel out volume units; result is mmol.ug 
## Multiply by 1000 to get to per mg 
## Multiply by 1000 to get to umol 

# TAC <- left_join(TAC, metadata) %>%
 # mutate(uae.mmol = uae.mmol.L * (vol_mL / 1000),
 #        uae.mmol.cm2 = uae.mmol / surface.area.cm2,
  #       uae.mmol.ugprot = uae.mmol.cm2 / prot_ug.mL) %>%
 # mutate(CRE.mmol = CRE.mmol.L * (vol_mL / 1000),
#         CRE.mmol.cm2 = CRE.mmol / surface.area.cm2,
 #        CRE.mmol.ugprot = CRE.mmol.cm2 / prot_ug.mL) %>%
 # mutate(CRE.uM.mgprot = CRE.mmol.L / prot_ug.mL)
```
