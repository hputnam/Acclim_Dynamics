...
Oct14 <- Oct14 %>% 
  ...
  mutate(Abs_net.adj = Abs_net - Oct14_st$Abs_net[Oct14_st$Standard == "Standard10"],
         uae.mM = map_dbl(Abs_net.adj, ~ predict(TAC_mod_lin3, newdata = data.frame(Abs_net.adj = .)))) #use the regression to calculate all the mean UAE values from the abs values
std_curve_plot3 + #plot the standard curve to check this against the protocol
...
Oct25 <- Oct25 %>% 
  ...
  mutate(Abs_net.adj = Abs_net - Oct25_st$Abs_net[Oct25_st$Standard == "Standard10"],
         uae.mM = map_dbl(Abs_net.adj, ~ predict(TAC_mod_lin4, newdata = data.frame(Abs_net.adj = .)))) #use the regression to calculate all the mean UAE values from the abs values
std_curve_plot4 + #plot the standard curve to check this against the protocol
...
# combine list 
Dilute.TAC.list <- union(Oct7dilute.tac, Oct14dilute.tac) %>% union(Oct25dilut.tac)
Dilute.TAC.list

##output as of 20201109 is only 1075 needs to be re-done.
```
...
# combine Oct7, Oct14, Oct25 files; filter out standard values
OctTAC <- union(Oct7, Oct14) %>% union(Oct25) 
OctTAC <- OctTAC[!grepl("Standard", OctTAC$Plug_ID),] 

OctTAC <- left_join(OctTAC, prot) %>% select(Plug_ID, uae.mM, prot_mg.mL) %>%
   mutate(CRE.umol.L = uae.mM*2189, # Step 1 of calculation described above
         CRE.umol = CRE.umol.L*0.00002, # Step 2
         prot.mg.L = prot_mg.mL*1000, # Step 3
         CRE.uM.mg = CRE.umol/prot.mg.L) # Step 4 
...
TAC_final <- OctTAC %>% select(Plug_ID, CRE.uM.mg) 
