...
CRIMP2.data <-
  list.files(path = 'data/Environmental/CRIMP2-buoy', pattern = ".csv", full.names = TRUE) %>% # list files in directory following a particular pattern
...
crimp.colors <- c("blue", "lightblue", "black", "salmon", "red3")
...
EXPT.temp <- Hobo.means %>% dplyr::select(Date.Time, Treatment, Temperature) %>% 
  dplyr::rename(Data.Source = Treatment, Temp = Temperature)
EXPT.temp$file.ID <- "EXPT"
CRIMP2.EXPT.temp <- bind_rows(CRIMP2.temp, EXPT.temp) #combining both dataframes 

CRIMP2.EXPT.temp.df2 <- CRIMP2.EXPT.temp %>%
  separate(Date.Time, sep=" ", c("Date", "Time")) %>%
  filter(!is.na(Temp)) %>%
  group_by(Date) %>%
  mutate(min = min(Temp),
         max = max(Temp),
...
CRIMP2.EXPT.temp.df2.stats <- summarySE(CRIMP2.EXPT.temp.df2, measurevar = "range")
...
ggsave(file="output/Figures/CRIMP2-Temperature.png", CRIMP.TEMP.PLOT, width = 11, height = 6, units = c("in"))
...
EXPT.pCO2 <- read.csv("data/Environmental/Seawater_chemistry_table_Output_All.csv") %>% select(Timepoint, Treatment, pCO2) %>% 
...
ggsave(file="Output/Figures/CRIMP2-pCO2.png", CRIMP.pCO2.PLOT, width = 11, height = 6, units = c("in"))
```
...
```{r}
tankIDs <- read.csv('metadata/Tank_to_Treatment.csv')
tankIDs$Tank <- as.character(tankIDs$Tank)
  
#bind all hobo logger data sets together
Hobo.data <-
  list.files(path = 'data/Environmental/Temp_Hobo_Loggers', pattern = ".csv", full.names = TRUE) %>% # list files in directory following a particular pattern
  set_names(.) %>% # get the column names
...
file.name <- c("data/Environmental/Temp_Hobo_Loggers/20190117_Tank1.csv", 
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank2.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank3.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank4.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank5.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank6.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank7.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank8.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank9.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank10.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank11.csv",
               "data/Environmental/Temp_Hobo_Loggers/20190117_Tank12.csv")
...
Hobo.daily <- Hobo.data %>% 
  separate(Date.Time, c("Date", "Time"), sep = " ", remove=FALSE) %>% 
  filter(!is.na(Temperature)) %>%
  filter((Date.Time > as.POSIXct("2018-09-22 06:50:00", tz="UTC") & Date.Time < as.POSIXct("2018-11-17 08:00:00", tz="UTC")))
...
write_csv(file = "data/Environmental/HOBO-Temperature-Treatment.csv", Hobo.treatment)
write_csv(file = "data/Environmental/HOBO-Temperature-Date.csv", Hobo.date)
```
...
write_csv(path = "data/Environmental/Hobologger-Treatment.csv", dt.trt.avg.hobo)
write_csv(path = "data/Environmental/Hobologger-Date.csv", dt.date.avg.hobo)
```
...
```{r}
dt.date.avg.hobo %>% dplyr::select(-sd, -ci) %>%
  filter(Date > "2018-09-21") %>%
  mutate(across(where(is.numeric), ~ round(., 3))) %>%
  unite(., "Value", Temperature, se, sep = " ± ", remove = TRUE) %>%
  unite(., "Value", Value, N, sep = ", n=", remove=TRUE) %>%
  spread(Treatment, Value) %>%
  write_csv(path = "data/Environmental/Summary_HOBO_temperature.csv")
