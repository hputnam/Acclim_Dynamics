...
Apex.Temp <- full_join(Apex.Temp, Trt.Tanks, by="Tank") %>% filter(Temp>20)

#### stuck on filtering temperature to below 20 -- could because the column is a character?
Apex.Temp <- Apex.Temp %>% filter(Apex.Temp, Temp > 20)
...
APEX.TEMP.PLOT <- ggplot(data = Apex.Temp, aes(x = Date.Time, y = Temp, color = Treatment)) +
  ...
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  #scale_x_datetime(date_breaks = "2 days", labels = date_format("%b %y")) +
