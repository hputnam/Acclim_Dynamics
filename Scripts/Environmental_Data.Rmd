...
2. [Continuous Data: pH (Apex), Temperature (Hobo)](#apex)
3. [Light (Apex and Licor) Data](#light)
4. [Flow Rates](#flow)
...
```{r}
Hobo_path = "Environmental_data/Temp_Hobo_Loggers/"
hobo_files <- list.files(path = Hobo_path, pattern = "*.csv") 

df <- tibble(file = hobo_files) %>%
  separate(file, into = c("Date", "Tank"), remove = FALSE)




filenames <- list.files("Environmental_data/Temp_Hobo_Loggers", pattern="*.csv", full.names=TRUE)
tbl <- sapply(filenames, read_csv, simplify=FALSE) %>% bind_rows(.id = "id")
