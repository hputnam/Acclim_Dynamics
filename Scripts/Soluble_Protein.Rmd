...
## 20201007 3 Plates
Oct7protein <- Oct7 %>% filter(!grepl("Standard", Plug_ID)) %>%
  filter(!is.na(Plug_ID)) %>%
  group_by(Plug_ID) %>%
  summarise(Abs480 = mean(Abs480)) %>% 
...
std_curve_plot2 + 
  geom_point(data = Oct7protein, aes(x = prot_ug.mL, y = Abs480.adj), pch = "X", cex = 5, alpha = 0.3) +
...
protein <- union(protein1, Oct7protein) %>% union(Oct14protein)
