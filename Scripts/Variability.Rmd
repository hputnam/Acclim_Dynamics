...
ggplot(phys.mad, aes(x=Treatment, y=Value, color=Treatment)) + geom_boxplot() + facet_grid(Response~Measurement) + theme_bw()
...
Resp.MAD <- ggplot(resp.mad, aes(x=Timepoint, y = mad, group = Treatment, color = Treatment)) +
  ...
  xlab("Timepoint") + ylab("Median Absolute Deviation") +
  theme_classic() + 
...
ggsave(file="Output/Final_Figures/MAD-Resp.png", Resp.MAD, width = 11, height = 6, units = c("in"))
```
...
CHLMAD <- ggplot(chl.mad, aes(x=Timepoint, y = mad, group = Treatment, color = Treatment)) +
  ...
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + ylab("Median Absolute Deviation") + ggtitle("Chlorophyll c2 Content") +
  theme_classic() + 
  theme(legend.position = "left") +
...
ggsave(file="Output/Final_Figures/MAD-AFDW.png", PROT.TAC.MAD, width = 11, height = 6, units = c("in"))
```
