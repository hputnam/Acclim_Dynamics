...
mcap.multi.phys.mad <- mcap.multi.phys %>% select(c(10:15))

pact.multi.phys <- multi.phys.mad %>% filter(Species=="Pacuta") 
pact.multi.phys.mad <- pact.multi.phys %>% select(c(10:15))

#Scale and center datasets
Mcap.data.scaled <- scale(mcap.multi.phys.mad, center = T, scale = T) # scaled variables
Pact.data.scaled <- scale(pact.multi.phys.mad, center = T, scale = T) # scaled variables
...
Come back to the bias.adjust - adjusts for small sample size bias (but what is small in this context?)

20210308 - stuck on converting distances to dataframe for the following MAD calculation. 
```{r}
##Mcap 
Mcap.disper <- betadisper(Mcap.dist, mcap.multi.phys$Group, type = c("median","centroid"), bias.adjust = FALSE)
...
Mcap.out$Plug_ID <- mcap.multi.phys$Plug_ID

%>% full_join(Mcap.out, mcap.multi.phys, by='row.names')
