...
phys.mad <- phys %>% select(-chla.ug.cm2) %>% unite(Big.Group, Group, Species, remove = FALSE, sep = " ") %>%
  subset(Big.Group != "Recovery HTAC Pacuta") %>% gather("Response", "Response.Value", 12:20) %>%
...
mcap.multi.phys.info <- mcap.multi.phys %>% select(1:10)
mcap.multi.phys.data <- mcap.multi.phys %>% select(c(11:16))

pact.multi.phys <- multi.phys.mad %>% filter(Species=="Pacuta") 
pact.multi.phys.data <- pact.multi.phys %>% select(c(11:16))
...
```{r}
## Day 1 ATAC
oneday.ATAC <- mcap.multi.phys %>% subset(Group == "Day 1 ATAC") %>% select(c(11:16))
