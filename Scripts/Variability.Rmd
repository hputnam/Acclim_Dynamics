...
Plot CV 
```{r}
cv.subset <- phys.mad %>% filter(cv<200)
...
CV.subset <- ggplot(cv.subset, aes(x=Timepoint, y = cv, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  facet_grid(Species ~ Response) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + ylab("Coefficient of Variation (%)") + ggtitle("Subset CV") +
  theme_classic() + 
  theme(legend.position = "left") +
  geom_vline(xintercept = c(7.2), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  ...
  panel.grid.minor = element_blank(),
  # Change axis line
  axis.line = element_line(colour = "black")); CV.subset

ggsave(file="Output/Final_Figures/CV-total.png", total.CV, width = 11, height = 10, units = c("in"))
ggsave(file="Output/Final_Figures/CV-subset.png", total.CV, width = 11, height = 10, units = c("in"))
```
...
ggsave(file="Output/Final_Figures/MAD-Resp.png", Resp.MAD, width = 11, height = 10, units = c("in"))
```
