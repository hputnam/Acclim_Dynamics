...
otu <- read.delim(file = "data/ITS2_seq/SymPortal_Andromeda_2022_Output/20220706T084046/its2_type_profiles/6_HoloInt1_20220706T084046.profiles.absolute.abund_and_meta.txt", sep = "\t", header = T, skip = 5)

test <- read.csv(file = "data/ITS2_seq/SymPortal_Local_Computer_2020_Output/SymPortal/FULL_ITS2/pre_med_seqs/pre_med_absolute_abundance_df.csv")
test <- test %>%
...
meta2 <- read.csv("data/Environmental/Master_Fragment.csv", header=T) %>%
...
cladeinfo <- read.delim2("~/MyProjects/Acclim_Dynamics_molecular/data/samples_Pacuta.annotations.txt", 
                         header=TRUE, sep="\t", na.strings=c("","NA")) %>% dplyr::rename(., Plug_ID = plugid) %>% 
  mutate_if(., is.integer, as.character) %>% 
  dplyr::select(., Plug_ID, ploidy, group, treesplit) %>%
  dplyr::mutate(., treesplit = coalesce(treesplit, "D")) #changing NA values to "D" in treesplit -- this is just so triploidy is split in 2 groups and diploidy is altogether 
...
NMDS_its2_plot2 <- nmds_its2_df %>% 
  ...
  ggplot(., aes(x=NMDS1, y=NMDS2, shape=treesplit, color=treesplit, fill=treesplit)) + 
  geom_point(size=2) + 
  theme_classic() +
...
ggsave(file="~/MyProjects/Acclim_Dynamics_molecular/data/ITS2/ITS2-NMDS-Mcap.png", NMDS_its2_plot2, width = 5, height = 4, units = c("in"))
...
ITS2_ploidy_plot <- ITS2_ploidy %>%
  ...
  labs(x="", y="Relative Abundance") +
  scale_fill_brewer(palette="Spectral") +
  facet_wrap(~treesplit, scales= "free_x") +
  theme_bw() + 
...
ITS2_ploidy_indivplot <- ITS2_ploidy %>%
  ...
  facet_grid(~treesplit, scales = "free",) +
...
ITS2_ploidy_permanova <- ITS2_ploidy %>% spread(Clade, relabund)
ITS2_ploidy_permanova.scaled <- scale(ITS2_ploidy_permanova[ ,15:23])

# PERMANOVA
adonis2(ITS2_ploidy_permanova.scaled ~ treesplit, data = ITS2_ploidy_permanova, method='eu',
        na.rm=TRUE)
```
