...
otu <- read.delim(file = "ITS2_seq/SymPortal_Andromeda_2022_Output/20220706T084046/its2_type_profiles/6_HoloInt1_20220706T084046.profiles.absolute.abund_and_meta.txt", sep = "\t", header = T, skip = 5)
...
meta <- read.csv("ITS2_seq/R_Input/Sample_variables.csv") %>% dplyr::rename(sample_name = sample)
meta2 <- read.csv("Environmental_data/Master_Fragment.csv", header=T) %>%
...
```{r}
rel_abund <- rel_its2_meta %>%
  ggplot(., aes(x = Timepoint, y = Clade)) + 
  geom_tile(aes(fill = relabund), color = "grey") +
  scale_fill_distiller(palette = "Greys", direction=100, labels = scales::label_percent(scale=1)) + 
  theme_classic() +
...
```{r readdata}
# read in otu and taxonomic outputs from QIIME2 
otu_2020 <- read.delim(file = "ITS2_seq/SymPortal_Local_Computer_2020_Output/SymPortal/FULL_ITS2/its2_type_profiles/3_first_analysis_2020-04-01_12-26-53.145538.profiles.absolute.abund_and_meta.txt", sep = "\t", header = T, skip = 5)
...
# only doing timepoint below because timepoint alone was significant
pairwise.results <- pairwise.adonis(its2.scaled,factors=its2_pca$Timepoint)
pairwise.results <- pairwise.results %>% filter(p.value < 0.05)
...
```{r}
relab_pca <- rel_its2_meta %>% select(1:3) %>% spread(Clade, relabund)
...
relab_pca_output <- full_join(relab_pca_output, rel_its2_meta, by = "sample_name")

pc_data_matrix <- pca.relabundance.results$x %>% as.data.frame
pc_meta_matrix <- relab_pca_output[,11:22]
pc_meta_matrix <- pc_meta_matrix %>% select(-Clade, -relabund)
...
pca_save <- relab_pca_output %>%
  ggplot(., aes(x=PC1,y=PC2)) + 
  stat_ellipse(aes(lty = Host_species), lwd = 0.5) +
  geom_point(aes(color=Treatment, shape = Host_species)) +
  theme_classic() +
...
biplot.pca <- ggbiplot(pca.out, choices = 1:2, pc.biplot = TRUE, labels = row.names(pca.out), 
...
sp.plot <- autoplot(pca.out, data = pc_meta_matrix, groups = 'Host_species', 
...
NMDS_its2_plot <- nmds_its2_df %>% filter(!is.na(Treatment)) %>%
  ggplot(., aes(x=NMDS1, y=NMDS2)) + 
  stat_ellipse(aes(lty = Host_species)) +
  geom_point(aes(shape=Host_species, color=Treatment)) + theme_classic() +
  scale_color_manual(values = c("blue", "lightblue", "salmon", "red3"), aesthetics = c("colour")) 

ggsave(file="Output/Manuscript_figures/ITS2-NMDS.png", NMDS_its2_plot, width = 7, height = 6, units = c("in"))
...
## PCoA unifrac distances between C profiles

```{r}
c_pcoa <- read.csv(file = "ITS2_seq/SymPortal_Andromeda_2022_Output/20220706T084046/between_profile_distances//C/20220706T084046_unifrac_profiles_PCoA_coords_C_no_sqrt.csv", sep = ",", header = T) %>% subset(!sample == "proportion_explained")

c_pcoa %>%
  ggplot(., aes(x=PC1, y=PC2)) + geom_point() + theme_classic()
```
