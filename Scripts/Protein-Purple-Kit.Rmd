...
prot.means <- prot %>% summarySE(measurevar="prot_mg.cm2", groupvars=c("Timepoint", "Treatment", "Temperature", "CO2", "Species")) %>%
  ...
  )) %>% subset(!Timepoint == "Day 1" & !Timepoint == "Day 2" & !Timepoint == "12 week" & !Timepoint == "16 week")

prot.means$Timepoint <- factor(prot.means$Timepoint, levels=c("1 week","2 week","4 week","6 week","8 week"))
...
```{r}
Pacuta_prot_LMER <- lmer(log(prot_mg.cm2) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=prot.pacuta)  
Mcap_prot_LMER <- lmer(log(prot_mg.cm2) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=prot.mcap)  
Mcap_prot_rec_LMER <- lmer(log(prot_mg.cm2) ~ Timepoint*Temperature*CO2 + (1|Tank), na.action=na.omit, data=prot.mcap.recovery)  
...
tab_model(Pacuta_prot_LMER, Mcap_prot_LMER, Mcap_prot_rec_LMER,
          dv.labels = c("P.acuta protein", "M. capitata protein Stress", "M.cap prot Recovery"), string.ci = "Conf. Int (95%)",
          string.p = "P-Value", file = "Output/Statistics/Protein.doc")

capture.output(pacuta.prot.anova, file = "Output/Statistics/Stress_statistics/Protein_Pacuta.txt")
capture.output(mcap.prot.anova, file = "Output/Statistics/Stress_statistics/Protein_Mcap.txt")
capture.output(mcap.prot.rec.anova, file = "Output/Statistics/Recovery_statistics//Protein_Mcap_recovery.txt")
