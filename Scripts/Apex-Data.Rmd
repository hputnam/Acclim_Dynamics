...
Trt.Tanks <- read.csv('metadata/Tank_to_Treatment.csv') %>% select(-Tank.Name)
...
APEX.data1 <- list.files(path = 'data/Environmental/Apex_continuous_data/Download1', pattern = ".csv", full.names = TRUE) %>% # list files in directory following a particular pattern
  ...
  select(-V1) %>% 
  row_to_names(row_number=1) %>% rename(file.ID = "data/Environmental/Apex_continuous_data/Download1/Experimental_0922.csv") %>%
  filter(Date.Time != "Date.Time")

APEX.data2 <- list.files(path = 'data/Environmental/Apex_continuous_data/Download2', pattern = ".csv", full.names = TRUE) %>% # list files in directory following a particular pattern
  set_names(.) %>% # get the column names
  map_dfr(read.table, .id = "file.ID", header=FALSE, sep=",") %>% # join all files together in one data frame by file ID
  group_by(file.ID) %>% # for each file do the following
  select(-V1) %>%
  row_to_names(row_number=1) %>% rename(file.ID = "data/Environmental/Apex_continuous_data/Download2/Experimental_0925_0926.csv") %>%
  filter(Date.Time != "Date.Time")
...
write_csv(path = "data/Environmental/Apex-Temperature-Treatment.csv", apex.temp.avg.trt)
write_csv(path = "data/Environmental/Apex-pH-Treatment.csv", apex.pH.avg.trt)

write_csv(path = "data/Environmental/Apex-Temperature-Date.csv", apex.temp.avg.date)
write_csv(path = "data/Environmental/Apex-pH-Date.csv", apex.pH.avg.date)
```
...
Light_tank_info <-read.csv('metadata/Tank_to_Treatment.csv', header=T, sep=",") %>% select(-Tank.Name)
...
write_csv(path = "data/Environmental/Apex-PAR.csv", APEX.PAR)
...
summarySE(Licor_LICOR193, measurevar="PAR")
...
## BY TREATMENT
light1 <- ggplot(Licor_LICOR193, aes(x=Treatment, y=PAR, color=Treatment)) + 
  scale_color_manual(values = treatment_colors) +
  geom_boxplot(size=1) +
  theme_bw() + 
  geom_jitter(size=2, width=0.3, alpha=0.3) +
  theme(legend.position="none", plot.title = element_text(size=11)) + 
  xlab("Treatment") + 
  ylab("PAR (nm)")

ggsave(filename = "output/Figures/LICOR-PAR.png", light1, device = "png", width = 6, height = 4)
