...
daily.plot <- ggplot(data = apex.pH.daily, aes(x = Time, y = pH, color = Treatment)) +
...
ggsave(filename = "Output/Apex-daily-pH-plot.png", daily.plot, device = "png", width = 5, height = 3)
```
...
Licor_date <- Licor_bytank %>% unite(Date.Time, Date, Time, sep = " ")
#Licor_bytank$Date.Time <- as.POSIXct(Licor_bytank$Date.Time, tz = "UTC")
...
licor_anova_tank <- aov(PAR ~ Position*Tank, data = Licor_bytank)
summary(licor_anova_tank)
```
...
```{r}
treatment_colors <- c("lightblue", "blue", "salmon", "red3")
