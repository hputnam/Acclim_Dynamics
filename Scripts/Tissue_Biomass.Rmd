...
Master_fragment <- read.csv("Environmental_data/Master_Fragment.csv") %>% filter(!ANALYSIS == "Molecular") %>% filter(!ANALYSIS == "Bleaching") %>% filter(!ANALYSIS == "NA") %>% 
  filter(!ANALYSIS == "Molecular Melted") %>% filter(!ANALYSIS == "Molecular BB") %>% filter(!ANALYSIS == "Physiology BB") %>% # need to keep "Physiology" and "Physiology/Molecular"
...
3. 16 missing corals contribute to the wide ranges at 1, 2 days and 1 week.
4. 2863 - redo this point 
