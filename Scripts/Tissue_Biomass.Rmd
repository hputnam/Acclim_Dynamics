...
#   select(Plug_ID, Species, Treatment, Temperature, CO2, Timepoint, Tank, partner, AFDW.mg.cells.cm2) %>% subset(partner == "Sym")
  #spread(partner, AFDW.mg.cells.cm2) %>% gather("partner", "value", 7:9) %>% na.omit()

AFDW.cells <- AFDW %>% select(-X) %>% distinct() %>% na.omit() %>%
  select(Plug_ID, Species, Treatment, Temperature, CO2, Timepoint, Tank, partner, AFDW.mg.cells) %>% subset(partner == "Sym") %>%
  spread(partner, AFDW.mg.cells) %>% filter(!Plug_ID == "1454")
...
AFDW.means.cells <- summarySE(AFDW.cells, measurevar = c("Sym"), groupvars = c("Treatment", "Timepoint", "Species"))
...
AFDW %>% select(Plug_ID, Species, Treatment, Temperature, CO2, Timepoint, partner, AFDW.mg.cm2, AFDW.mg.cells) %>%
  write_csv(path = "Output/Tissue_Biomass.csv")

AFDW.cm2 %>% write_csv(path = "Output/Tissue_Biomass_Ratio.csv")
```
...
```{r}
### CM2 - SURFACE AREA 
AFDW.means.cm2_plot_full <- ggplot(AFDW.means.cm2, aes(x=Timepoint, y = value, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  facet_grid(partner~Species, scales = "free") +
  geom_errorbar(aes(ymin=value-se, ymax=value+se), width=.2) +
  scale_color_manual(values = cols) + 
  ...
  theme(legend.position = c(0.92,0.57)) +
  geom_vline(xintercept = c(5.2), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1.2, hjust = 1.3)) + #Set the text angle
  theme(panel.border = element_blank(),
  ...
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0))) +
  theme(axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0))) +
  theme(legend.title = element_text(size = 12)); AFDW.means.cm2_plot_full # Change axis line

ggsave(file="Output/Supplemental_figures/fullAFDWcm2.png", AFDW.means.cm2_plot_full, width = 10, height = 9, units = c("in"))

#### CELLS 
AFDW.means.cells_plot_full <- ggplot(AFDW.means.cells, aes(x=Timepoint, y = Sym, group = Treatment, color = Treatment)) +
  geom_line() + geom_point() +
  facet_grid(~Species, scales = "free") +
  geom_errorbar(aes(ymin=Sym-se, ymax=Sym+se), width=.2) +
  scale_color_manual(values = cols) + 
  xlab("Timepoint") + 
  theme_classic() + 
  ylab(expression("AFDW" ~(mg ~cell^-1))) +
  theme(legend.position = c(0.92,0.57)) +
  geom_vline(xintercept = c(5.2), colour="black", linetype="dotted") +
  theme(plot.title = element_text(face = 'bold.italic', size = 12, hjust = 0)) + 
  theme(axis.text.x = element_text(angle = 60, vjust = 1.2, hjust = 1.3)) + #Set the text angle
  ...
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0))) +
  theme(axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0))) +
  theme(legend.title = element_text(size = 12)); AFDW.means.cells_plot_full # Change axis line

ggsave(file="Output/Supplemental_figures/fullAFDWcells.png", AFDW.means.cells_plot_full, width = 11, height = 6, units = c("in"))
```
...
biomass.mcap <- biomass %>% subset(Species == "Montipora capitata") %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week") 

biomass.mcap.recovery <- biomass %>% subset(Species == "Montipora capitata") %>% subset(Timepoint == "12 week" | Timepoint == "16 week")

hist(biomass.pacuta$Host) 
hist(biomass.mcap$Host) 
hist(biomass.mcap.recovery$Host) 

hist(biomass.pacuta$Sym) 
hist(biomass.mcap$Sym) 
hist(biomass.mcap.recovery$Sym) 

hist(biomass.pacuta$Ratio_S.H) 
hist(biomass.mcap$Ratio_S.H) 
hist(biomass.mcap.recovery$Ratio_S.H) 

biomass.ttests <- biomass %>% subset(Timepoint == "1 week")
```
...
Pacuta_Host_LMER <- lmer(Host ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.pacuta, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Host_LMER <- lmer(Host ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Host_recovery_LMER <- lmer((Host) ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap.recovery, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

qqPlot(residuals(Pacuta_Host_LMER)) 
qqPlot(residuals(Mcap_Host_LMER)) 
#qqPlot(residuals(HOST_LMER))
qqPlot(residuals(Mcap_Host_recovery_LMER))

hist(residuals(Pacuta_Host_LMER)) 
hist(residuals(Mcap_Host_LMER)) 
#hist(residuals(HOST_LMER)) 
hist(residuals(Mcap_Host_recovery_LMER)) 
...
mcap.host.recovery.anova <- Anova(Mcap_Host_recovery_LMER, ddf="lme4", type='III') 

capture.output(pacuta.host.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Host_Pacuta.txt")
capture.output(mcap.host.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Host_Mcap.txt")
capture.output(mcap.host.recovery.anova, file = "Output/Statistics/Recovery_Statistics/AFDW_Host_Mcap_Recovery.txt")

species.t <- t.test(Host~Species, data = biomass.ttests)
capture.output(species.t, file = "Output/Statistics/Species_comparison/AFDW_Host.txt")
```

#### Sym AFDW  cm-2

```{r}
Pacuta_Symcm2_LMER <- lmer(Sym ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.pacuta) 

Mcap_Symcm2_LMER <- lmer(Sym ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Symcm2_recovery_LMER <- lmer(Sym ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap.recovery, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

qqPlot(residuals(Pacuta_Symcm2_LMER)) 
qqPlot(residuals(Mcap_Symcm2_LMER)) 
qqPlot(residuals(Mcap_Symcm2_recovery_LMER)) 

hist(residuals(Pacuta_Symcm2_LMER)) 
hist(residuals(Mcap_Symcm2_LMER)) 
hist(residuals(Mcap_Symcm2_recovery_LMER)) 

## Type III Wald chisquare tests
pacuta.sym.anova <- Anova(Pacuta_Symcm2_LMER, ddf="lme4", type='III') 
mcap.sym.anova <- Anova(Mcap_Symcm2_LMER, ddf="lme4", type='III') 
mcap.sym.recovery.anova <- Anova(Mcap_Symcm2_recovery_LMER, ddf="lme4", type='III') 

capture.output(pacuta.sym.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Symcm2_Pacuta.txt")
capture.output(mcap.sym.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Symcm2_Mcap.txt")
capture.output(mcap.sym.recovery.anova, file = "Output/Statistics/Recovery_statistics/AFDW_Symcm2_Mcap_Recovery.txt")

species.t.sym <- t.test(Sym~Species, data = biomass.ttests)
capture.output(species.t.sym, file = "Output/Statistics/Species_comparison/AFDW_Sym.txt")
```

#### Sym AFDW  cells-1

```{r}
AFDW.cells.pacuta <- AFDW.cells %>% subset(Species == "Pocillopora acuta") %>% 
  subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week")

AFDW.cells.mcap <- AFDW.cells %>% subset(Species == "Montipora capitata") %>% subset(Timepoint == "1 week" | Timepoint == "2 week" | Timepoint == "4 week" | Timepoint == "6 week" | Timepoint == "8 week") 

AFDW.cells.mcap.recovery <- AFDW.cells %>% subset(Species == "Montipora capitata") %>% subset(Timepoint == "12 week" | Timepoint == "16 week")

AFDW.cells.ttest <- AFDW.cells %>% subset(Timepoint == "1 week")

Pacuta_Sym_LMER_cells <- lmer(log(Sym) ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=AFDW.cells.pacuta, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Sym_LMER_cells <- lmer(log(Sym) ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=AFDW.cells.mcap, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Sym_recovery_LMER_cells <- lmer(log(Sym) ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=AFDW.cells.mcap.recovery, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

qqPlot(residuals(Pacuta_Sym_LMER_cells)) 
qqPlot(residuals(Mcap_Sym_LMER_cells)) 
qqPlot(residuals(Mcap_Sym_recovery_LMER_cells)) 

hist(residuals(Pacuta_Sym_LMER_cells)) 
hist(residuals(Mcap_Sym_LMER_cells)) 
hist(residuals(Mcap_Sym_recovery_LMER_cells)) 

## Type III Wald chisquare tests
pacuta.cells.sym.anova <- Anova(Pacuta_Sym_LMER_cells, ddf="lme4", type='III') 
mcap.cells.sym.anova <- Anova(Mcap_Sym_LMER_cells, ddf="lme4", type='III') 
mcap.cells.sym.recovery.anova <- Anova(Mcap_Sym_recovery_LMER_cells, ddf="lme4", type='III') 

capture.output(pacuta.cells.sym.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Sym_cells_Pacuta.txt")
capture.output(mcap.cells.sym.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Sym_cells_Mcap.txt")
capture.output(mcap.cells.sym.recovery.anova, file = "Output/Statistics/Recovery_statistics/AFDW_Sym_Mcap_cells_Recovery.txt")

species.t.sym.cells <- t.test(Sym~Species, data = AFDW.cells.ttest)
capture.output(species.t.sym.cells, file = "Output/Statistics/Species_comparison/AFDW_Sym_cells.txt")
```
...
```{r}
Pacuta_Ratio_LMER <- lmer(log(Ratio_S.H) ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.pacuta, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Ratio_LMER <- lmer(Ratio_S.H ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 

Mcap_Ratio_LMER_recovery <- lmer(Ratio_S.H ~ Timepoint*Temperature*CO2 + (1|Tank:Temperature:Timepoint) + (1|Tank:CO2:Timepoint), na.action=na.omit, data=biomass.mcap.recovery, contrasts = list(Timepoint = "contr.sum", Temperature = "contr.sum", CO2 = "contr.sum")) 
...
capture.output(pacuta.ratio.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Ratio_Pacuta.txt")
capture.output(mcap.ratio.anova, file = "Output/Statistics/Stress_statistics/AFDW/AFDW_Ratio_Mcap.txt")
capture.output(mcap.ratio.recovery.anova, file = "Output/Statistics/Recovery_statistics/AFDW_Ratio_Mcap_recovery.txt")

species.t.ratio <- t.test(`Ratio_S.H`~Species, data = biomass.ttests)
capture.output(species.t.ratio, file = "Output/Statistics/Species_comparison/AFDW_Ratio.txt")
```
