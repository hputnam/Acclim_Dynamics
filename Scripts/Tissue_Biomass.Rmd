...
cell.density <- read.csv("Output/cell_counts.csv") %>% select(Plug_ID, haemo.cells.cm2)
...
AFDW.cm2 <- AFDW %>% select(-X) %>% distinct() %>% select(Plug_ID, partner, Species, Treatment, Temperature, CO2, Timepoint, AFDW.mg.cm2) %>% spread(partner, AFDW.mg.cm2) %>% mutate(Ratio_S.H=Sym/Host)

AFDW.cells.cm2 <- AFDW %>% select(-X) %>% distinct() %>% select(Plug_ID, partner, Species, Treatment, Temperature, CO2, Timepoint, AFDW.mg.cells.cm2) %>% spread(partner, AFDW.mg.cells.cm2) 

#### left off at trying to consolidate the above 2 dataframes and make a column for measurement 

# Taking out 1116 because it was an outlier. 
# filter(AFDW.final, Ratio_S.H>3)
week4ATHC <- AFDW.final %>% subset(Timepoint == "4 week" & Species == "Pocillopora acuta" & Treatment == "ATHC")

## Calculating AFDW / cells.cm2
AFDW.final <- AFDW.final %>% mutate(AFDW.mg.cells.cm2 = ((dry.bioimass.g-burnt.bioimass.g)*1000) / haemo.cells.cm2)


# Aggregating by Host, Sym, Ratio
AFDW.final <- AFDW.final %>% gather("partner", "value", 9:11) %>% na.omit()

# Calculating means for each group and filtering out those groups with sample size less than 3
AFDW.means <- summarySE(AFDW.final, measurevar = c("value"), groupvars = c("Treatment", "Timepoint", "Species", "partner"))
AFDW.means <- AFDW.means %>% filter(N > 3) %>%
...
AFDW.means$partner <- factor(AFDW.means$partner, levels=c("Host", "Symbiont", "S:H Ratio"))
```
...
## Plotting
```{r}
AFDW_plot <- ggplot(AFDW.means, aes(x=Timepoint, y = value, group = Treatment, color = Treatment)) +
