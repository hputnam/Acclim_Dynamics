...
Mc.km_fit <- survfit(Surv(lifespan, status) ~ group, data=Mc.Data)
#MC.sur <- summary(Mc.km_fit, times = c(1:58))
#Mc.km_fit <- survfit(Surv(lifespan, status) ~ 1, data=Mc.Data)
#autoplot(Mc.km_fit)
# Fig.MC.sur <- ggsurvplot(Mc.km_fit, data = Mc.Data, pval = TRUE)

pdf("Output/Mcap_Survivorship.pdf", onefile=FALSE)
#par(mfrow=c(1,1))
ggsurvplot(Mc.km_fit,  data=Mc.Data,size = 1,  # change line size
           linetype = "strata", # change line type by groups
           break.time.by = 7, # break time axis
           #palette = c("lightblue", "blue","pink", "red"), # custom color palette
...
Pa.km_fit <- survfit(Surv(lifespan, status) ~ Treatment*Origin, data=Pa.Data)
Pa.sur <- summary(Pa.km_fit, times = c(1:58))
#autoplot(Pa.km_fit)
#Fig.PA.sur <- ggsurvplot(Pa.km_fit, data = Pa.Data, pval = TRUE)

ggsurvplot(Pa.km_fit, data=Pa.Data, size = 1,  # change line size
           linetype = "strata", # change line type by groups
           break.time.by = 7, # break time axis
           palette = c("lightblue", "blue","pink", "red"), # custom color palette
           conf.int = TRUE, # Add confidence interval
